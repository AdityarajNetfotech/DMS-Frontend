...
function SidebarThreeBg({ accentColor = 0x059669 }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Compliance sidebar Three.js background skipped:", err);
    }
  }, [accentColor]);
...
export default function ComplianceSidebar() {
  ...
  const [branding, setBranding] = useState(() => {
    ...
    return { logo: "", primaryColor: "#059669", companyName: "Compliance Workspace" };
  });

  const [userProfile, setUserProfile] = useState(() => {
    const email = localStorage.getItem("userEmail") || "compliance.team@dms.io";
    const rawName = email.split("@")[0].replace(".", " ").replace(/\b\w/g, (c) => c.toUpperCase());
    const name = localStorage.getItem("userName") || rawName || "Compliance Team";
    return { name, email };
  });
  ...
  const menuItems = [
    {
      name: t("complianceDashboard"),
      icon: LayoutDashboard,
      path: `${slugPrefix}/compliance/dashboard`,
    },
    {
      name: t("complianceApprovals"),
      icon: FileCheck2,
      path: `${slugPrefix}/compliance/approvals`,
      badge: pendingApprovalCount > 0 ? pendingApprovalCount : null
    },
    {
      name: t("profileSettings"),
      icon: UserCheck,
      path: `${slugPrefix}/compliance/profile-settings`,
    },
  ];

  const primaryColorStr = branding?.primaryColor || "#059669";
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-[260px] bg-white/70 backdrop-blur-md border-r border-emerald-100 flex-col h-screen sticky top-0 overflow-hidden relative">
        ...
        <div className="relative z-10 flex flex-col h-full w-full">
          ...
          <div className="px-6 py-5 border-b bg-white/40">
            <div className="flex items-center gap-3">
              <div 
                style={!branding.logo ? { backgroundColor: branding.primaryColor || '#059669' } : {}}
                className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold overflow-hidden border border-emerald-100 shadow-sm"
              >
                {branding.logo ? (
              ...
              <div>
                ...
                <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200/60 mt-0.5">
                  <ShieldCheck size={11} /> {t("complianceOfficer")}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4">
            <p className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">{t("complianceReview")}</p>
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <NavLink
                    ...
                    to={item.path}
                    style={({ isActive }) =>
                      isActive ? { backgroundColor: branding.primaryColor || '#059669', color: '#fff' } : {}
                    }
                    className={({ isActive }) =>
            ...
            <div className="mt-6 p-4 rounded-2xl bg-gradient-to-br from-emerald-950 via-teal-950 to-slate-900 text-white shadow-md border border-emerald-700/50">
              ...
              <p className="text-xs text-white font-bold mt-2">{t("complianceVerification")}</p>
              <p className="text-[11px] text-white mt-1 leading-relaxed">
                <strong className="text-white font-extrabold">{pendingApprovalCount}</strong> {t("complianceQueueDesc")}
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between p-2 rounded-xl bg-white/60 border border-slate-100 shadow-sm">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="truncate">
                  ...
                  <p className="text-[10px] text-slate-500 truncate">{t("complianceOfficer")}</p>
                </div>
              </div>
      ...
      <button
        ...
        className="lg:hidden fixed bottom-4 right-4 z-50 p-3.5 bg-emerald-600 text-white rounded-full shadow-xl"
        ...
        <div className="lg:hidden fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-sm" onClick={() => setIsOpen(false)}>
          <div className="w-[260px] h-full bg-white p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4">
              ...
              <span className="text-xs text-emerald-600 font-semibold">{t("complianceOfficer")}</span>
            </div>
            <nav className="space-y-1 flex-1 overflow-y-auto">
              {menuItems.map((item) => (
                <NavLink
                  ...
                  className="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium text-slate-700 hover:bg-emerald-50"
                >
                  <div className="flex items-center gap-3">
                    <item.icon size={18} />
                    <span>{item.name}</span>
              ...
              <div className="mt-4 p-3.5 rounded-xl bg-gradient-to-br from-emerald-950 via-teal-950 to-slate-900 text-white shadow-md border border-emerald-700/50">
                <div className="flex items-center justify-between">
                  <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-white/10 text-white border border-white/20">
                    {t("complianceVerification")}
                  </span>
                  <Sparkles size={12} className="text-white animate-pulse" />
                </div>
                <p className="text-xs text-white font-bold mt-1.5">{t("complianceVerification")}</p>
