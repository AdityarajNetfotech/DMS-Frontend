...
function SidebarThreeBg({ accentColor = 0x7c3aed }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Legal sidebar Three.js background skipped:", err);
    }
  }, [accentColor]);
...
export default function LegalSidebar() {
  ...
  const [branding, setBranding] = useState(() => {
    ...
    return { logo: "", primaryColor: "#7c3aed", companyName: "Legal Workspace" };
  });

  const [userProfile, setUserProfile] = useState(() => {
    const email = localStorage.getItem("userEmail") || "legal.team@dms.io";
    const rawName = email.split("@")[0].replace(".", " ").replace(/\b\w/g, (c) => c.toUpperCase());
    const name = localStorage.getItem("userName") || rawName || "Legal Team";
    return { name, email };
  });
  ...
  const menuItems = [
    {
      name: t("legalDashboard"),
      icon: LayoutDashboard,
      path: `${slugPrefix}/legal/dashboard`,
    },
    {
      name: t("legalApprovals"),
      icon: Scale,
      path: `${slugPrefix}/legal/approvals`,
      badge: pendingApprovalCount > 0 ? pendingApprovalCount : null
    },
    {
      name: t("profileSettings"),
      icon: UserCheck,
      path: `${slugPrefix}/legal/profile-settings`,
    },
  ];

  const primaryColorStr = branding?.primaryColor || "#7c3aed";
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-[260px] bg-white/70 backdrop-blur-md border-r border-purple-100 flex-col h-screen sticky top-0 overflow-hidden relative">
        ...
        <div className="relative z-10 flex flex-col h-full w-full">
          ...
          <div className="px-6 py-5 border-b bg-white/40">
            <div className="flex items-center gap-3">
              <div 
                style={!branding.logo ? { backgroundColor: branding.primaryColor || '#7c3aed' } : {}}
                className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold overflow-hidden border border-purple-100 shadow-sm"
              >
                {branding.logo ? (
                  <img src={branding.logo} alt="Logo" className="w-full h-full object-contain" />
                ) : (
                  (branding.companyName || "L").charAt(0).toUpperCase()
                )}
              </div>
              ...
              <div>
                ...
                <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-purple-700 bg-purple-50 px-2 py-0.5 rounded-md border border-purple-200/60 mt-0.5">
                  <Scale size={11} /> {t("legalTeam")}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4">
            <p className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">{t("legalReview")}</p>
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <NavLink
                    ...
                    to={item.path}
                    style={({ isActive }) =>
                      isActive ? { backgroundColor: branding.primaryColor || '#7c3aed', color: '#fff' } : {}
                    }
                    className={({ isActive }) =>
            ...
            <div className="mt-6 p-4 rounded-2xl bg-gradient-to-br from-purple-950 via-indigo-950 to-slate-900 text-white shadow-md border border-purple-700/50">
              ...
              <p className="text-xs text-white font-bold mt-2">{t("legalVerification")}</p>
              <p className="text-[11px] text-white mt-1 leading-relaxed">
                <strong className="text-white font-extrabold">{pendingApprovalCount}</strong> {t("legalQueueDesc")}
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between p-2 rounded-xl bg-white/60 border border-slate-100 shadow-sm">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="truncate">
                  ...
                  <p className="text-[10px] text-slate-500 truncate">{t("legalTeam")}</p>
                </div>
              </div>
      ...
      <button
        ...
        className="lg:hidden fixed bottom-4 right-4 z-50 p-3.5 bg-purple-600 text-white rounded-full shadow-xl"
        ...
        <div className="lg:hidden fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-sm" onClick={() => setIsOpen(false)}>
          <div className="w-[260px] h-full bg-white p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4">
              ...
              <span className="text-xs text-purple-600 font-semibold">{t("legalTeam")}</span>
            </div>
            <nav className="space-y-1 flex-1 overflow-y-auto">
              {menuItems.map((item) => (
                <NavLink
                  ...
                  className="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium text-slate-700 hover:bg-purple-50"
                >
                  <div className="flex items-center gap-3">
                    <item.icon size={18} />
                    <span>{item.name}</span>
              ...
              <div className="mt-4 p-3.5 rounded-xl bg-gradient-to-br from-purple-950 via-indigo-950 to-slate-900 text-white shadow-md border border-purple-700/50">
                <div className="flex items-center justify-between">
                ...
                <p className="text-xs text-white font-bold mt-1.5">{t("legalVerification")}</p>
