...
export default function SearchPage() {
  ...
  const fetchFavorites = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/${companySlug}/viewer/favorites`, {
        headers: {
      ...
      if (resData.success) {
        const formatted = resData.data.map(doc => {
          ...
          return {
            ...
            sharedBy: doc.uploadedBy || "System",
            initials: getInitials(doc.uploadedBy || "System"),
        ...
        setError(resData.message || "Failed to load favorites.");
      }
    } catch (err) {
      console.error(err);
      setError("Network error fetching starred documents.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFavoriteToggle = async (documentId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/${companySlug}/viewer/favorites/${documentId}`, {
        ...
        body: JSON.stringify({ isFavorite: false }), // Remove from favorites
      });
        ...
        <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-base font-medium text-slate-500">
              Documents and folders you have starred as favorites.
            </p>
          </div>
                ...
                <table className="w-full min-w-[1080px] table-fixed text-left">
                  <thead>
                    <tr className="border-b border-slate-200 text-sm font-semibold text-slate-500">
                      <th className="w-[30%] py-5 pr-5">Name</th>
                      <th className="w-[10%] px-5 py-5">Type</th>
                      <th className="w-[20%] px-5 py-5">Shared By</th>
                      <th className="w-[20%] px-5 py-5">Folder</th>
