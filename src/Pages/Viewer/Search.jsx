...
export default function SearchPage() {
  ...
  const fetchSearchResults = async (searchQuery) => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/${companySlug}/viewer/search?q=${encodeURIComponent(searchQuery)}`, {
        headers: {
      ...
      if (resData.success) {
        const formatted = (resData.data.documents || []).map(doc => {
          ...
          return {
            ...
            sharedBy: doc.uploadedBy?.name || "System",
            initials: getInitials(doc.uploadedBy?.name || "System"),
            folder: doc.folderId?.name || "Root",
        ...
        setError(resData.message || "Failed to load search results.");
      }
    } catch (err) {
      console.error(err);
      setError("Network error fetching search results.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: String(new Set(documents.map(d => d.type)).size),
  ...
  const typeOptions = ["All Types", "PDF", "DOCX", "PPTX", "XLSX", "IMAGE", "ZIP"];
  ...
  const dateOptions = ["Date Added", "Newest", "Oldest"];
  ...
  const handleClearFilters = () => {
    ...
    setDateFilter("Date Added");
  };

  const handleExportList = () => {
    if (!filteredDocuments.length) return;
    
    const headers = ["Name", "Type", "Uploaded By", "Folder", "Date Added"];
      ...
      <div className="mx-auto max-w-[1500px] space-y-7">
        <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-base font-medium text-slate-500">
              {q ? `Showing results for "${q}"` : "All accessible documents"}
            </p>
          </div>
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto px-5">
            {loading ? (
              <p className="text-center py-10 text-slate-500 font-semibold">Searching documents...</p>
            ) : (
              <>
                <table className="w-full min-w-[1080px] table-fixed text-left">
                  <thead>
                    <tr className="border-b border-slate-200 text-sm font-semibold text-slate-500">
                      ...
                      <th className="w-[17%] px-5 py-5">Uploaded By</th>
                      <th className="w-[17%] px-5 py-5">Folder</th>
                      <th className="w-[13%] px-5 py-5">Date Added</th>
                ...
                {filteredDocuments.length === 0 && (
                  <p className="text-center py-10 text-slate-500 font-medium">No documents found matching "{q}".</p>
                )}
              </>
            )}
          </div>
