...
export default function SharedFolderDetailManager() {
  ...
  const fetchFolderContents = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/${companySlug}/manager/folders/${folderId}`, {
        headers: {
      ...
      if (resData.success) {
        ...
        setUploadPermission(resData.data.folder?.uploadAllowed || false);
  ...
  const handleDownload = (documentId, name) => {
    ...
    window.open(`${API_BASE_URL}/api/${companySlug}/manager/documents/${documentId}/download?token=${token}`, "_blank");
  };

  const handlePreview = (documentId) => {
    const token = localStorage.getItem("accessToken");
    window.open(`${API_BASE_URL}/api/${companySlug}/manager/documents/${documentId}/preview?token=${token}`, "_blank");
  };
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-500">
              <Link to={`/${companySlug}/manager/shared-with-me`} className="hover:text-blue-600 flex items-center gap-1">
                <ArrowLeft size={16} />
                Shared with me
              </Link>
              <span>/</span>
                      ...
                      >
                        <td className="py-4 pr-5">
                          <div className="flex items-center gap-4 min-w-0">
                            ...
                            {item.isFolder ? (
                              <button
                                onClick={() => navigate(`/${companySlug}/manager/shared-folders/${item._id}?uploadAllowed=${uploadPermission}`)}
                                className="truncate hover:text-blue-600 hover:underline cursor-pointer font-semibold text-left"
                                title={item.name}
                              >
                                {item.name}
                              </button>
                            ) : (
                              <span className="truncate" title={item.originalFileName || item.name}>
                        ...
                        <td className="px-5 py-4 font-semibold">
                          <span className={`rounded px-2.5 py-1 text-xs font-bold ${item.typeColor}`}>
                            {item.type}
                          </span>
                        </td>
                        <td className="px-5 py-4 text-slate-500">{item.meta}</td>
                        <td className="py-4 pl-5 text-right">
                          <div className="flex justify-end gap-1.5">
                            {!item.isFolder && (
                              <button
                                onClick={() => handleDownload(item._id, item.originalFileName || item.name)}
