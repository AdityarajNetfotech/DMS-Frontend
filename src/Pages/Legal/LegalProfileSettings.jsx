...
import { Eye, EyeOff, User, Lock, Scale, FileSignature } from "lucide-react";
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        onChange={onChange}
        disabled={disabled}
        className="mt-2 h-12 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm font-medium text-slate-900 outline-none transition focus:border-purple-500 focus:ring-2 focus:ring-purple-100 disabled:bg-slate-50 disabled:text-slate-500"
      />
    </label>
  );
}
    ...
    <div className="block">
      ...
      <div className="relative mt-2">
        <input
          ...
          value={value}
          onChange={onChange}
          className="h-12 w-full rounded-lg border border-slate-200 bg-white px-4 pr-12 text-sm font-medium text-slate-900 outline-none transition placeholder:text-slate-500 focus:border-purple-500 focus:ring-2 focus:ring-purple-100"
        />
        <button
          type="button"
...
export default function LegalProfileSettings() {
  ...
  const [role, setRole] = useState("Legal Team");
        ...
        <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight text-slate-950 md:text-3xl mt-2">
              Legal Profile & Security
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              Manage your legal counsel profile, digital electronic signature, and security password.
            </p>
          </div>
        </section>
          ...
          <div className="flex gap-6 sm:gap-8 px-1 overflow-x-auto">
            ...
            >
              <span className="flex items-center gap-2">
                <User size={16} /> Legal Officer Profile
              </span>
              {isProfileTab ? (
                <span className="absolute inset-x-0 bottom-[-1px] h-0.5 rounded-full bg-purple-700" />
              ) : null}
            </button>
        ...
        {isProfileTab && (
          <form onSubmit={handleSaveProfile} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-lg font-bold text-slate-950">
              Personal & Legal Contact Details
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              These credentials appear on signed legal audits and dual maker-checker certificates.
            </p>
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-8 inline-flex h-12 min-w-[200px] items-center justify-center rounded-xl bg-purple-700 px-8 text-sm font-bold text-white shadow-sm transition hover:bg-purple-800 disabled:opacity-50 cursor-pointer"
            >
        ...
        {isPasswordTab && (
          <form onSubmit={handleChangePassword} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Ensure your password adheres to enterprise security standards for legal authorization roles.
            </p>
